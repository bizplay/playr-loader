...
call :IS_PLAYR_BROWSER_RUNNING
if not "%playr_browser_running%"=="1" exit /b 0
...
if errorlevel 1 (
  echo %date% %time% WARNING: Playr browser CPU time unchanged over %watchdog_browser_check_interval_in_sec%s; killing and restarting %browser_process_name%>> "%playr_log%"
...
call :RESOLVE_POWERSHELL_EXE
if defined powershell_exe (
  "%powershell_exe%" -NoProfile -Command "$p=$env:PLAYR_PROFILE_DIR; $n=$env:BROWSER_PROCESS; $f=$false; Get-WmiObject Win32_Process -Filter ('Name='''+$n+'''') -ErrorAction SilentlyContinue | ForEach-Object { if($_.CommandLine -and ($_.CommandLine.Contains($p) -or $_.CommandLine.Contains('playr_loader.html'))){ $f=$true } }; if($f){exit 0}else{exit 1}" >nul 2>nul
